...
class UniformQuantize(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        # print('{} | {} | {}'.format(qmin, qmax, num_bits))
        # print(scale, zero_point)
        

        min_scale = torch.tensor(1e-8).expand_as(scale).to(scale.dtype).to(device)
        ...
        with torch.no_grad():
            ...
            # print('input', input)
            # print('output', output)
            # print('{} | {} | {} | {}'.format(qmin, qmax, num_bits, scale))
            # print('qparams', qparams)
            # print('norm', torch.norm(input))
...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, input, num_bits, num_grad_bits):
        ...
        if self.bias is not None:
            bias_qparams = calculate_qparams(
                self.bias, num_bits=num_bits, flatten_dims=(0, -1), reduce_dim=None)
...
class QLinear(nn.Linear):
    ...
    def forward(self, input, num_bits=0, num_bits_grad=0):
